...
org.make_derived('corr_loadings2', {'corr_loadings'},...
    @(x)x.^2);

org.make_derived('corr_loadings_shuf2', {'corr_loadings_shuf'},...
    @(x)x.^2);
...
org.make_derived('corr_loadings_ipr', {'corr_loadings'},...
    @(l) sum(l.^2).^2 ./ sum(l.^4));
...
for d = [1:40, 83]
    ...
    name = sprintf('corr_delta_cos2_area_%d',d);
    unshuf = sprintf('corr_cos2_area_%d',d);
    shuf = [unshuf '_shuf'];
    
    org.make_derived(unshuf, {'corr_loadings'},...
    @(c)up_to(c.^2, d));
    
    org.make_derived(shuf, {'corr_loadings_shuf'},...
    @(c)up_to(c.^2, d));
...
org.make_derived('cos2_area_abs', {'loadings', 'loadings_shuf'},...
    @(l,ls)sum(abs(l.^2 - ls.^2)));
org.make_derived('corr_cos2_area_abs', {'corr_loadings', 'corr_loadings_shuf'},...
    @(l,ls)sum(abs(l.^2 - ls.^2)));

org.make_derived('cos4_area', {'loadings'},...
    @(l) sum(l.^4));
org.make_derived('corr_cos4_area', {'corr_loadings'},...
    @(l) sum(l.^4));
...
efn_func = @(x) sum(x.^2,1).^2 ./ sum(x.^4,1);
...
org.make_derived('N10', {'loadings2'},...
    @(l2)find(cumsum(l2)>0.1,1));
