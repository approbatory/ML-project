function [B, b0, lambda, stats] = LassoRegress(X,y,varargin)
% [B, b0, lambda, stats] = LassoRegress(X,y,varargin)
%
...
for i = 1:length(lambda)
  ...
  while(1)
    [b1,active1] = cyclicCoordinateDescent(X,y,b,Xss,active,lambda(i),predictorWeights);
    if (verbose)
    ...
    if (converged)
      ...
      [b2,active2] = cyclicCoordinateDescent(X,y,b1,Xss,potentiallyActive,lambda(i),predictorWeights);      
...
function [b, active] = cyclicCoordinateDescent(X,y,b,Xss,active,lambda,predictorWeights)
% Helper function for performing cyclic coordinate descent.
%
active(predictorWeights==0) = false;
if (any(active))
  r = y - X(:,active)*b(active);
  for currActive = find(active)
    bold = b(currActive);
    rj = r + X(:,currActive)*b(currActive); % Compute the partial residual
    bj = 2*(rj'*X(:,currActive)); 
    b(currActive) = sign(bj)*max(abs(bj) - lambda/predictorWeights(currActive), 0)/(2*Xss(currActive)); % Soft thresholding
    active(currActive) = (b(currActive)~=0);
    % Update residual to reflect the new value of bj
    r  = r + X(:,currActive)*(bold - b(currActive));
  end
end
...
active(predictorWeights==0) = false;
r = y - X(:,active)*b(active);
active = abs(2*r'*X)>(lambda./predictorWeights');
