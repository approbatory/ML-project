function [B, b0, lambda, stats] = LassoRegress2(X,y,varargin)
% [B, b0, lambda, stats] = LassoRegress2(X,y,varargin)
%
% A faster version of LASSOREGRESS, that uses Covariance Updates.
% 
...
if (any(active))
  ...
  for i = 1:numel(ind)
    ...
    bj = 2*(Xy(i) + Xss(i)*b(i)-XX(i,:)*b);
    b(i) = max(abs(bj) - lambda/predictorWeights(i), 0)*sign(bj)/(2*Xss(i)); % Soft threshold
