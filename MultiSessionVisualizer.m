...
classdef MultiSessionVisualizer
    methods(Static)
        ...
        function MedLoad
            ...
            Utils.bns_groupings(rate_f, rate_f_s, rate_f_conf, rate_f_s_conf, mouse_name);
            xlabel 'Mouse index'
            ylabel 'Fit exponent'
            Utils.specific_format('MBNS');
        ...
        function SnN(normed, filt_num)
            ...
            figure;
            Utils.bns_groupings(sm_slope, sms_slope, sm_conf, sms_conf, mouse_list);
            xlabel 'Mouse index';
            ...
            [~,~,select_indices] = DecodeTensor.special_sess_id_list;
            if filt_num
                select_indices_filt = false(1,107);
            ...
            errorbar(I0_fit_value(g_).*N_fit_value(g_), 1./sm_slope(g_), inv_sm_slope_uncertainty(g_), inv_sm_slope_uncertainty(g_),...
                limit_uncertainty(g_), limit_uncertainty(g_), 'LineStyle', 'none', 'Color', 'b', 'CapSize', 1);
            ...
            errorbar(I0_fit_value_s(g_), 1./sms_intercept(g_), inv_intercept_errb(g_), inv_intercept_errb(g_), I0_upper_s(g_), I0_upper_s(g_), 'LineStyle', 'none', 'Color', 'r', 'CapSize', 1);
