...
% TODO: add x-errbars using 1.96.*std(bootstrp(1e5, @var, fwhm))
rng(0);

data = var_widths_vs_nsv;
make_xlsx(data, 'f5f');
end

%% helper functions
function data = var_widths_vs_nsv
org = Org;
org.init;

figure;
data = my_correlogram(org, 'var_fwhm_cm', 'signal_var_norm', true, true, true);
