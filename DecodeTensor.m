...
%                 tracesEvents.spikeML = tracesEvents.spikeML(351:end,:);
    ...
    methods(Static) %appendix
        ...
        function command = build_command_sess(sess, setting, error_type,...
            ...
            assert(iscell(sess) && numel(sess) == 2, 'Supply sess as {"Mouse20xx", "12345"}');
            bp = 'and MinDist = 0'; %boilerplate
            if isempty(num_neurons) && isempty(num_trials)
                command = sprintf(['select NumNeurons, DataSize,'...
                    ' %s from decoding where Mouse = ''%s'' and SessionID = ''%s'' and'...
            ...
            if isempty(num_neurons) && strcmp(num_trials, 'max')
                command = sprintf(['select NumNeurons, DataSize,'...
                    ...
                    ' where Mouse = ''%s'' and SessionID = ''%s'' and Setting = ''%s'') %s'...
            ...
            if strcmp(num_neurons, 'max') && strcmp(num_trials, 'max')
                command = sprintf(['select NumNeurons, DataSize,'...
                    ' %s from decoding where Mouse = ''%s'' and SessionID = ''%s'' and'...
                    ' Setting = ''%s'' and DataSize ='...
                    ' (select max(DataSize) from decoding'...
                    ' where Mouse = ''%s'' and SessionID = ''%s'' and Setting = ''%s'') and NumNeurons ='...
                    ' (select max(NumNeurons) from decoding'...
                    ' where Mouse = ''%s'' and SessionID = ''%s'' and Setting = ''%s'') %s'...
                    ' order by NumNeurons, DataSize'],...
                    error_type, sess{:}, setting, sess{:}, setting, sess{:}, setting, bp);
            end
            command = [command ';'];
        end
    ...
    methods
        ...
        function [mean_cell_snr, sem_cell_snr] = single_cell_d_primes2(o)
            %[N, K, T] = size(o.data_tensor);
            signal_leftward = diff(mean(o.data_tensor(:,:,o.tr_dir==-1), 3, 'omitnan'), 1, 2).^2;
            signal_rightward = diff(mean(o.data_tensor(:,:,o.tr_dir==1), 3, 'omitnan'), 1, 2).^2;
            
            
            noise_leftward = var(o.data_tensor(:,:,o.tr_dir==-1), [], 3, 'omitnan');
            noise_leftward = (noise_leftward(:, 1:end-1) + noise_leftward(:, 2:end))/2;
            
            noise_rightward = var(o.data_tensor(:,:,o.tr_dir==1), [], 3, 'omitnan');
            ...
            mean_over_bins = mean([dp2_leftward dp2_rightward], 2, 'omitnan'); %used to be median
            mean_cell_snr = mean(mean_over_bins, 'omitnan');
